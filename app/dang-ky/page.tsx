  ...
  useEffect(() => {
    if (refCode) {
      fetch(`/api/referral?code=${refCode}`)
        .then((r) => r.json())
        .then((d) => {
          if (d.success) setReferrerName(d.referrerName);
        })
        .catch(() => {});
    }
  }, [refCode]);
    ...
    >
      <div className="bg-white rounded-3xl shadow-xl p-8 relative overflow-hidden border border-slate-100">
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 rounded-xl border transition-all outline-none font-body ${
                phoneError ? "border-red-300 focus:ring-red-500" : "border-slate-200 focus:ring-red-500"
        ...
        <p className="mt-8 text-center text-sm text-slate-600 font-body relative z-10">
          Đã có tài khoản?{" "}
          <Link href="/dang-nhap" className="text-red-600 font-bold hover:underline">
            Đăng Nhập
          </Link>
        </p>
      </div>
