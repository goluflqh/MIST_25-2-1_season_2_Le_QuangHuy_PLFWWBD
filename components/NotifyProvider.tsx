...
export function NotifyProvider({ children }: { children: React.ReactNode }) {
  ...
  const showConfirm = useCallback((message: string, onConfirm: () => void, confirmLabel?: string) => {
    ...
    setConfirm({ message, onConfirm, label: confirmLabel || (isDeleteAction ? "Xoá" : "Xác nhận") });
  }, []);
  ...
  const isDestructive = confirm ? isDeleteActionText(confirm.label) : false;
        ...
        >
          ...
          >
            <h2 id={titleId} className="mb-2 font-heading text-lg font-extrabold text-slate-950">
              {isDestructive ? "Xác nhận xoá" : "Xác nhận thao tác"}
            </h2>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                className={`min-h-11 rounded-xl px-5 py-2 font-body text-sm font-bold text-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${isDestructive ? "bg-red-600 hover:bg-red-700 focus-visible:ring-red-600" : "bg-slate-900 hover:bg-slate-800 focus-visible:ring-slate-700"}`}
              >
                {confirm.label}
              </button>
            </div>
