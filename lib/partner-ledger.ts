...
function parseLedgerAmount(value: unknown, allowNegative = false) {
  ...
  const amount = Number.parseInt(normalized, 10) * sign;
  if (!Number.isFinite(amount) || amount === 0) return null;
  if (!allowNegative && amount < 0) return null;
  return amount;
}

export function getPartnerEntrySignedAmount(entry: { amount: number; entryType: string }) {
...
export function getPartnerBalance(entries: Array<{ amount: number; entryType: string }>) {
